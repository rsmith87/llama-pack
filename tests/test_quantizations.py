...
def test_quantization_manager_excludes_mmproj_gguf_files(tmp_path):
    hf_dir = tmp_path / "HFModels"
    model_dir = hf_dir / "qwen-vl"
    model_dir.mkdir(parents=True)
    source = model_dir / "qwen.gguf"
    source.write_bytes(b"1234")
    (model_dir / "mmproj-F16.gguf").write_bytes(b"1234")
    (model_dir / "qwen-mmproj.gguf").write_bytes(b"1234")
    make_quantize_binary(tmp_path / "llama.cpp" / "build" / "bin" / "llama-quantize")
