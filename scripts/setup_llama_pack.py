...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--env-file", default=str(ROOT_DIR / ".llama_pack.env"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Interactive Llama Pack setup wizard for controllers and agents."
    )
...
def controller_steps(args: argparse.Namespace, config: str | None = None) -> list[Step]:
    ...
    if args.start:
        steps.append(Step("Start controller", ["scripts/start_controller.sh"], {"LLAMA_PACK_ENV_FILE": args.env_file}))
    return steps
...
def agent_steps(args: argparse.Namespace, config: str | None = None, port: str | None = None) -> list[Step]:
    ...
    if args.controller_registration_key:
        env["LLAMA_PACK_CONTROLLER_REGISTRATION_KEY_OUTBOUND"] = args.controller_registration_key
    ...
    if args.start:
        steps.append(
            Step(
                ...
                {"LLAMA_PACK_ENV_FILE": args.env_file, "LLAMA_PACK_PORT": agent_port},
            )
        )
    return steps
...
def build_steps(args: argparse.Namespace) -> list[Step]:
    ...
    if not args.skip_uv_sync:
        steps.append(Step("Install Llama Pack Python dependencies", ["uv", "sync"]))
...
def main() -> int:
    ...
    print("\nLlama Pack setup complete." if not args.dry_run else "\nDry run complete.")
    if args.role in {"controller", "single-machine"}:
