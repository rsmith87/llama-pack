...
from llama_pack.core.plugins.hooks import HookRegistry, PolicyHookRejected
...
class ChatScheduler:
    ...
    async def chat_with_meta(self, model_name: str, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
        ...
        try:
            ...
            await self._emit("llama_pack.chat.rejected", {
        ...
        try:
            ...
            raise
        await self._emit("llama_pack.chat.admitted", {"model": model_name, "session_id": session_id})
        ...
        try:
            ...
            await self._emit("llama_pack.chat.completed", {
                "model": model_name,
            ...
            await self._emit("llama_pack.chat.failed", {
        ...
        async def _release_after_stream() -> AsyncIterator[bytes]:
            try:
                ...
                if event_bus is not None:
                    await event_bus.emit("llama_pack.chat.completed", payload={
                        "model": model_name,
                        "session_id": session_id,
                        "prompt_tokens": 0,
                        "completion_tokens": 0,
                ...
                if event_bus is not None:
                    await event_bus.emit("llama_pack.chat.failed", payload={
                        "model": model_name, "session_id": session_id, "error": str(exc),
                    })
                raise
            finally:
