...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m llama_pack.auth")
    parser.add_argument(
        "--config",
        help="Path to a Llama Pack config file. Defaults to LLAMA_PACK_CONFIG/config.yaml/config.example.yaml.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "create-test-chat-key":
        ...
        print(f"Key ID: {created['id']}")
        print(f"API key: {created['key']}")
        print("Set LLAMA_PACK_TEST_CHAT_API_KEY to this value for /ui/test-chat bootstrap injection.")
