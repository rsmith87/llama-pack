    ...
    if any(isinstance(item, ssl.SSLCertVerificationError) for item in chain):
        ...
        if _contains_any(
            ...
            return (
                ...
                "Reissue the certificate for the exact hostname used in the Neuraxis URL."
            )
        if _contains_any(
            text,
            (
            ...
            return (
                ...
                "Install the local CA root and set SSL_CERT_FILE/REQUESTS_CA_BUNDLE to the Neuraxis CA chain."
            )
