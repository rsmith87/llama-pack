...
class ClientAuthDiscovery(BaseModel):
    methods: list[str] = Field(default_factory=lambda: ["neuraxis_api_key", "external_api_key"])
...
class ClientDiscoveryResponse(BaseModel):
    product: Literal["neuraxis"] = "neuraxis"
    version: str
    mode: str
...
async def client_discovery(request: Request) -> ClientDiscoveryResponse:
    ...
    auth_methods = ["neuraxis_api_key", "external_api_key"]
    endpoints = ClientEndpointDiscovery()

    if business_enabled:
        auth_methods.append("neuraxis_business")
        endpoints.businessAuth = "/lm-api/v1/plugins/neuraxis_business/auth/login"
...
async def _business_plugin_available(request: Request) -> bool:
    ...
    record = next(
        (item for item in status_payload.get("plugins", []) if item.get("id") == "neuraxis_business"),
        None,
    )
